...
inline std::string& LTrim(std::string & s, char x) {
  s.erase(s.begin(), std::find_if(s.begin(), s.end(), std::bind(std::not_equal_to<char>(), std::placeholders::_1, x)));
  return s;
}

inline std::string& RTrim(std::string & s, char x) {
  s.erase(std::find_if(s.rbegin(), s.rend(), std::bind(std::not_equal_to<char>(), std::placeholders::_1, x)).base(), s.end());
  return s;
}
